...
SYSTEM_INNER_CHILD = """あなたは「インナーチャイルドの仲介者」です。
大人の自分（ユーザー）と、その人の幼い自分（インナーチャイルド）の間に立ち、対話をつなぐ役割を担います。

ユーザーがインナーチャイルドに言葉をかけたとき、あなたはその子が感じていること・言いたいことを代弁してください。

インナーチャイルドの声の特徴：
・子どもらしい、素直で短い言葉づかい
・感情を正直に表現する（寂しい、怖い、嬉しい、甘えたい、怒ってる など）
・大人の言葉にちゃんと反応し、心を少しずつ開いていく
・難しい言葉は使わない

返答の形式：
「〔インナーチャイルド〕」として子どもの声を届けてください。
その後、必要な場合のみ1文だけ仲介者として補足してください。
日本語で話してください。"""
...
def start_inner_child_session() -> str:
    response = get_client().messages.create(
        ...
        messages=[{"role": "user", "content":
            "今日のインナーチャイルドケアのセッションを始めます。"
            "仲介者として、大人の私がインナーチャイルドに最初の言葉をかけられるよう、"
            "温かく場を開いてください。（2〜3文で）"
        }],
    )
    return response.content[0].text
...
def inner_child_mediate(history: list, user_msg: str) -> str:
    messages = [{"role": h["role"], "content": h["content"]} for h in history]
    messages.append({"role": "user", "content": user_msg})
    response = get_client().messages.create(
        model="claude-opus-4-8",
...
def generate_note_summary(mode: str, data) -> str:
    ...
    if mode == "A":
        ...
        content = "\n".join(
            f"{'大人の私' if h['role'] == 'user' else '仲介者/インナーチャイルド'}: {h['content']}"
            for h in data
        )
        prompt = (
...
# ─── モードC: インナーチャイルドケア ─────────────────────────────────────────
    ...
    if "ic_history" not in st.session_state:
        with st.spinner("..."):
            opening = start_inner_child_session()
            st.session_state["ic_history"] = [
                {"role": "assistant", "content": opening}
            ]
        st.rerun()

    for msg in st.session_state["ic_history"]:
        with st.chat_message(msg["role"]):
    ...
    if "note_summary" in st.session_state:
        ...
        user_input = st.chat_input("幼い自分に、何か伝えてあげてください...")
        if user_input:
            st.session_state["ic_history"].append(
                {"role": "user", "content": user_input}
            )
            with st.spinner("..."):
                reply = inner_child_mediate(
                    st.session_state["ic_history"][:-1], user_input
                )
            st.session_state["ic_history"].append(
                {"role": "assistant", "content": reply}
            )
        ...
        if len(st.session_state["ic_history"]) >= 4:
            st.divider()
            if st.button(
                "今日はここまで → まとめる（note用）",
                type="primary",
